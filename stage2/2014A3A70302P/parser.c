...
void printparsetreenode( PTPNode node, PTPNode parent,int *slno) { 
	if ( node == NULL) { 
	...
	if ( IS_TERMINAL(node->symbol) ) {
		...
		printf("\033[033m");
		switch( node->symbol ) { 
			case T_NUM: 	printf("%-6d %-22d %-6d %-20s %-20s %-6s %-24s\n",(*slno)++, node->terminal.lexeme._int,node->linenumber,TerminalStrings[INDEX_TERMINAL(node->symbol)],NonTerminalStrings[parent->symbol],"YES",TerminalTokens[INDEX_TERMINAL(node->symbol)]); break;
			case T_RNUM: 	printf("%-6d %-22.2f %-6d %-20s %-20s %-6s %-24s\n",(*slno)++, node->terminal.lexeme._real,node->linenumber,TerminalStrings[INDEX_TERMINAL(node->symbol)],NonTerminalStrings[parent->symbol],"YES",TerminalTokens[INDEX_TERMINAL(node->symbol)]); break;
			case T_ID: printf("%-6d %-22s %-6d %-20s %-20s %-6s %-24s\n",(*slno)++, node->terminal.lexeme._id,node->linenumber,TerminalStrings[INDEX_TERMINAL(node->symbol)],NonTerminalStrings[parent->symbol],"YES",TerminalTokens[INDEX_TERMINAL(node->symbol)]); break;
			case T_FUNID: printf("%-6d %-22s %-6d %-20s %-20s %-6s %-24s\n",(*slno)++, node->terminal.lexeme._id,node->linenumber,TerminalStrings[INDEX_TERMINAL(node->symbol)],NonTerminalStrings[parent->symbol],"YES",TerminalTokens[INDEX_TERMINAL(node->symbol)]); break;
			case T_STR: printf("%-6d %-22s %-6d %-20s %-20s %-6s %-24s\n",(*slno)++, node->terminal.lexeme._str,node->linenumber,TerminalStrings[INDEX_TERMINAL(node->symbol)],NonTerminalStrings[parent->symbol],"YES",TerminalTokens[INDEX_TERMINAL(node->symbol)]); break;
			default: printf("%-6d %-22s %-6d %-20s %-20s %-6s %-24s\n",(*slno)++, TerminalSymbols[INDEX_TERMINAL(node->symbol)],node->linenumber,TerminalStrings[INDEX_TERMINAL(node->symbol)],NonTerminalStrings[parent->symbol],"YES",TerminalTokens[INDEX_TERMINAL(node->symbol)]); break;
		}
		printf("\033[0m");
	}
	else {
		...
		// MAY BE IT HAS NO CHILDREN (Parsing Error)
		if ( node->nonterminal.child == NULL )  //printf("%s %d NOCHILD\n",NonTerminalStrings[node->symbol], node->linenumber); 
			if (parent != NULL)	printf("%-6d %-22s %-6d %-20s %-20s %-6s %-24s\n", (*slno)++, "",node->linenumber,NonTerminalStrings[node->symbol],NonTerminalStrings[parent->symbol],"YES",NonTerminalTokens[node->symbol]);
			else printf("%-6d %-22s %-6d %-20s %-20s %-6s %-24s\n",*(slno)++, "",node->linenumber,NonTerminalStrings[node->symbol],"ROOT","YES",NonTerminalTokens[node->symbol]);
		
		else { 
			printf("\033[0m");
			// Print the first Child
			PTPNode tmp = node->nonterminal.child ; // Get the first child
			printparsetreenode(tmp,node, slno ); // Print the first child
			printf("\033[034m");		
			// Print the current node ...
			if (parent!= NULL) printf("%-6d %-22s %-6d %-20s %-20s %-6s %-24s\n",(*slno)++, "",node->linenumber,NonTerminalStrings[node->symbol],NonTerminalStrings[parent->symbol],"NO",NonTerminalTokens[node->symbol]);
			else printf("%-6d %-22s %-6d %-20s %-20s %-6s %-24s\n",(*slno)++, "",node->linenumber,NonTerminalStrings[node->symbol],"ROOT","NO",NonTerminalTokens[node->symbol]);
			printf("\033[0m");
			// Print rest of children
			while ( (tmp = tmp->next ) != NULL ) {
				// For each of other children
				printparsetreenode(tmp, node, slno );
			}
		}
...
void printparsetree() {
	printf("\n\n\t\t\t\033[035mPARSE TREE (IN-ORDER TRAVERSAL)\n" );
	for (int i=0; i < 96; i++) printf("="); printf("\n");
	printf("%-6s %-22s %-6s %-20s %-20s %-6s %-24s\n","SLNO","LEXEME","LINENO","TOKEN","PARENT","ISLEAF","SYMBOL");
	for (int i=0; i < 96; i++) printf("_"); printf("\033[0m\n");
	int slno = 1;
	printparsetreenode(PTRoot,NULL, &slno);
for (int i=0; i < 96; i++) printf("="); printf("\n");
}
...
int countparsetreenode( PTPNode node ) { 
	if ( node == NULL)return 0;// printf("WARNING: NULL NODE\n"); 
	//Leftmost child ‐‐> parent node‐‐> remaining siblings (excluding the leftmost)
	if ( IS_TERMINAL(node->symbol) ) {
		...
		// Non-terminal ...
		// MAY BE IT HAS NO CHILDREN (Parsing Error)
		if ( node->nonterminal.child == NULL ) 
			return 1; // only non terminal ...
		else { 
			int count = 1; // current node
			PTPNode tmp = node->nonterminal.child ; // Get the first child
			count += countparsetreenode(tmp); 
			while ( (tmp = tmp->next ) != NULL ) {
				// For each of other children
				count += countparsetreenode(tmp);
			}
...
bool parseInputFile(char *programfile) {
	// Note: If is able to parse the input .. (If in can insert and rectify then this is recoverable error...)
	bool recoverableparsing = true; // this flag is set if valid parse tree cannot be generated ... 
		...
		if ( IS_TERMINAL(top->symbol) ) {
			...
			if ( top->symbol == lookahead.token ) {
				...
				switch( lookahead.token ) {
					...
					case T_MAIN: // also for main add the id // <------------- ADDED LATER ...
				...
				while (true) {
					...
					if (lookahead.token == T_EOF) {
						...
						if (IS_NON_TERMINAL(top->symbol)) { // If top of stack is non-terminal set it to NULL
							...
							recoverableparsing = false; // Forcing Non-terminal to NULL may not work ...
	...
	// Use this to tell if program is syntactically correct
	return successfulparsing ; // This tells if program was syntactically correct or not ...
	// Use this to tell if parse was successfully generated ...
	// return recoverableparsing; 
}
// This does not delete the next of itseft 
// So separately delete the next of first node ...
void deleteparsetreenode(PTPNode node) {
	if ( node == NULL ) {
		// printf("Delete Parse Tree called with NULL..\n");
		return; 
	}
	if ( IS_TERMINAL(node->symbol) ){
		// printf("Deleting Terminal: %s\n", TerminalStrings[INDEX_TERMINAL(node->symbol)] );
	}
	else {
		// printf("Deleting NonTerminal: %s\n", NonTerminalStrings[node->symbol] );
	}
	if ( !IS_TERMINAL(node->symbol) ) {
		// If non-terminal...
		PTPNode tmp = node->nonterminal.child;
		while ( tmp != NULL ){
			PTPNode tmp1 = tmp->next;
			deleteparsetreenode(tmp);
			tmp = tmp1;
		}
	}
	free(node);
...
void deleteparsetree() {
	// printf("Deleting Parse Tree...\n");
	deleteparsetreenode(PTRoot);
	PTRoot = NULL ; // Setting the pointer to NULL
}

PTPNode getparsetree(){
	return PTRoot; // this is just a function to return parse tree ....
}
void printprogramnodefromparsetree( PTPNode node, int *indent ){
	if ( node == NULL ) return;
	if ( IS_TERMINAL(node->symbol) ) {
		switch ( node->symbol ){
			case T_END:
		...
		switch( node->symbol ) { 
			case T_NUM: printf("\033[031m"); printf(" %d",  node->terminal.lexeme._int);  break;
			case T_RNUM: printf("\033[032m"); printf(" %.2f", node->terminal.lexeme._real); break;
			case T_ID: printf("\033[032m"); printf(" %s",  node->terminal.lexeme._id ); break;
			case T_STR: printf("\033[034m"); printf(" %s", node->terminal.lexeme._str ); break;
			case T_FUNID: printf("\033[035m"); printf(" %s", node->terminal.lexeme._id); break;
			case T_LE: case T_LT: case T_GT: case T_GE: case T_EQ: case T_NE: case T_AND: case T_NOT: case T_OR:
			case T_PLUS: case T_MINUS: case T_MUL: case T_DIV: 
			printf("\033[036m"); printf(" %s",  TerminalSymbols[INDEX_TERMINAL(node->symbol)]); break;
			default: printf("\033[037m"); printf(" %s",  TerminalSymbols[INDEX_TERMINAL(node->symbol)]);
		}
		printf("\033[0m");
		switch ( node->symbol ){
			case T_FUNCTION:
		...
		PTPNode child = node->nonterminal.child;
		if ( child == NULL ) return;
		while ( child != NULL ){
			printprogramnodefromparsetree(child, indent);
			child = child->next;
		}
	}
}
void printprogramfromparsetree(){
	for ( int i =0 ; i < 20 ; i ++ )
		printf("===");
	printf("\n PARSE TREE: (PREORDER TRAVERSAL) GENERATED PROGRAM ..\n");
	int indent= 0;
	printf("\n");
	printf("%*s",(indent)*4,"");
	printf("\033[034m");
	printprogramnodefromparsetree(PTRoot, &indent);
	printf("\033[0m");
	printf("\n");
	for ( int i =0 ; i < 20 ; i ++ )
		printf("===");
